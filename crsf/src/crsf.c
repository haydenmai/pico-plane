 ...
 *
 * @param callback A function pointer to the callback function that takes an array of
 * uint16_t channels as input.
 */
 ...
 *
 * This function sets the callback function that will be called when link statistics are
 * available.
 *
 ...
 *
 * @param callback A function pointer to the callback function that takes a boolean
 * parameter indicating the failsafe status.
 */
 ...
 *
 * The link quality threshold determines the minimum acceptable link quality for CRSF
 * communication. A lower threshold allows for more frames to be lost before the failsafe
 * is triggered.
 *
 ...
 */
void crsf_set_rssi_threshold(uint8_t threshold) { _rssi_threshold = threshold; }

/**
 * Initializes the CRSF communication by setting up the UART and configuring the RX and TX
 * pins.
 *
 ...
 */
void crsf_end() { uart_deinit(_uart); }

void _process_rc_channels()
{
    const crsf_payload_rc_channels_packed_t *payload
        = (const crsf_payload_rc_channels_packed_t *)&_incoming_frame[3];
...
{
    const crsf_payload_link_statistics_t *link_stats_payload
        = (const crsf_payload_link_statistics_t *)&_incoming_frame[3];
    _link_statistics.rssi         = (link_stats_payload->diversity_active_antenna
                                         ? link_stats_payload->uplink_rssi_ant_2
...
{
    return _link_statistics.link_quality <= _link_quality_threshold
        || _link_statistics.rssi >= _rssi_threshold;
}
...
bool crsf_process_frame(uint8_t *frameIndex, uint8_t *frameLength, uint8_t *crcIndex,
                        uint8_t currentByte)
{
    // Frame format:
    ...
    if (*frameIndex == 0) {
        ...
        if (currentByte != 0xC8 && currentByte != 0xEE) {
            DEBUG_WARN("Invalid sync byte: %04X\n", currentByte);
            return false;
        }
 ...
 */
void crsf_telem_set_battery_data(uint16_t voltage, uint16_t current, uint32_t capacity,
                                 uint8_t percent)
{
