...
#define MSP_NAV_STATUS_ERROR_TOOFAR                                                      \
    1 // Next waypoint distance is more than safety distance
#define MSP_NAV_STATUS_ERROR_SPOILED_GPS                                                 \
    2 // GPS reception is compromised - Nav paused - copter is adrift !
#define MSP_NAV_STATUS_ERROR_WP_CRC                                                      \
    3 // CRC error reading WP data from EEPROM - Nav stopped
...
#define MSP_NAV_STATUS_ERROR_INVALID_DATA                                                \
    7 // Invalid mission step action code, aborting, copter is adrift
...
// MSP_RX_MAP reply
struct msp_rx_map_t {
    uint8_t rxmap[MSP_MAX_MAPPABLE_RX_INPUTS]; // [0]=roll channel, [1]=pitch channel,
                                               // [2]=yaw channel, [3]=throttle channel,
                                               // [3+n]=aux n channel, etc...
} __attribute__((packed));
...
// MSP_SET_WP command
// Special waypoints are 0 and 255. 0 is the RTH position, 255 is the POSHOLD position
// (lat, lon, alt).
struct msp_set_wp_t {
    ...
    int16_t p1; // speed (cm/s) when action is MSP_NAV_STATUS_WAYPOINT_ACTION_WAYPOINT, or
                // "land" (value 1) when action is MSP_NAV_STATUS_WAYPOINT_ACTION_RTH
    int16_t p2; // not used
    int16_t p3; // not used
...
class MSP {
  ...
  public:
    void begin(uart_inst_t *uart, uint32_t timeout = 500);
